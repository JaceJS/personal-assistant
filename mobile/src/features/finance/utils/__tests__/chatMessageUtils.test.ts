...
describe("withDateSeparators", () => {
  ...
  it("inserts one separator before a single day of messages", () => {
    const a = { ...createUserTextMessage("a"), createdAt: new Date("2026-01-05T09:00:00") };
    const b = { ...createUserTextMessage("b"), createdAt: new Date("2026-01-05T10:00:00") };
    const result = withDateSeparators([a, b]);
    expect(result).toHaveLength(3);
    expect(result[0]).toMatchObject({ type: "dateSeparator" });
    expect(result[1]).toBe(a);
    expect(result[2]).toBe(b);
  });

  it("inserts a separator whenever the calendar day changes", () => {
    const day1 = { ...createUserTextMessage("a"), createdAt: new Date("2026-01-05T09:00:00") };
    const day2a = { ...createUserTextMessage("b"), createdAt: new Date("2026-01-06T09:00:00") };
    const day2b = { ...createUserTextMessage("c"), createdAt: new Date("2026-01-06T10:00:00") };
    const day3 = { ...createUserTextMessage("d"), createdAt: new Date("2026-01-08T09:00:00") };
    const result = withDateSeparators([day1, day2a, day2b, day3]);
    const separatorIndices = result
      .map((item, i) => (item.type === "dateSeparator" ? i : -1))
      .filter((i) => i !== -1);
    expect(separatorIndices).toEqual([0, 2, 5]);
