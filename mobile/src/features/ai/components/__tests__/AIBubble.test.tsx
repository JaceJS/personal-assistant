...
import { createAITypingMessage, rejectAIMessage, resolveAIMessage } from '@/features/finance/utils/chatMessageUtils';

const noop = () => {};

describe('AIBubble', () => {
  it('shows a retry button when the message failed', async () => {
    const failed = rejectAIMessage(createAITypingMessage('sate 20.000'), 'Could not get a response.');
