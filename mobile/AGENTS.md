...
One repo, three build environments: local dev, staging, production. Two independent axes: **where env vars come from** and **which keystore signs the APK**.
...
| Staging (APK, internal test) | `eas build -p android --profile staging` | `eas.json` → `build.staging.env` | `https://savyn-api-staging.fly.dev` | EAS-managed keystore |
...
- **Staging points at the separate staging backend + Supabase project; production uses the prod backend + Supabase.**
