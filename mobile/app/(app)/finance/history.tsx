...
import { ChevronLeft, ChevronRight, Plus, Search, SlidersHorizontal, Wallet } from 'lucide-react-native';
...
export default function HistoryScreen() {
  ...
  const dateTo = new Date(selectedMonth.year, selectedMonth.month + 1, 0).toISOString().slice(0, 10);
  ...
  const { data, isLoading, isRefetching, refetch } = useTransactions({ dateFrom, dateTo, limit: 200 });
  ...
  return (
    <Screen>
      ...
      <View style={styles.rangeRow}>
        ...
        <Text style={[styles.periodTotal, { color: totalIsNegative ? colors.danger.text : colors.success.text }]}>
          Total: {totalIsNegative ? '−' : '+'}{formatRupiah(Math.abs(periodTotal))}
        </Text>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          isLoading
            ? <View style={styles.listPad}><SkeletonList count={5} /></View>
            : <EmptyState icon={Wallet} title="No transactions" subtitle="Nothing recorded this month" />
        }
