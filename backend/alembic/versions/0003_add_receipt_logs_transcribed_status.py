...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TYPE voice_processing_status ADD VALUE IF NOT EXISTS 'transcribed' AFTER 'transcribing'"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "processing_status", voice_processing_status, server_default="pending", nullable=False
        ),
    ...
    op.add_column(
        "voice_logs", sa.Column("account_id", postgresql.UUID(as_uuid=True), nullable=True)
    )
    op.drop_index(op.f("ix_voice_logs_user_created"), table_name="voice_logs")
    op.create_foreign_key(
        None, "voice_logs", "accounts", ["account_id"], ["id"], ondelete="SET NULL"
    )
...
def downgrade() -> None:
    ...
    op.create_index(
        op.f("ix_voice_logs_user_created"),
        "voice_logs",
    ...
    op.create_index(
        op.f("ix_transactions_user_occurred"),
        "transactions",
        ["user_id", sa.literal_column("occurred_at DESC")],
        unique=False,
    )
    op.create_index(
        op.f("ix_transactions_user_category_occurred"),
        "transactions",
        ["user_id", "category_id", sa.literal_column("occurred_at DESC")],
        unique=False,
    )
